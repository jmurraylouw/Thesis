...
\begin{figure}[ht]
    ...
    %   \path(ref_sum)+(\nodesep,0) node (ctrl_k) [simple_block] {$K$};
      ...
      \path(ctrl_i)+(\nodesep,0) node (ctrl_i_gain) [simple_block] {$I_{\Omega_j}$};
    %   \path(ref_sum)+(\nodesep,-1.5*\blockheight) node (ctrl_kd) [simple_block] {$K$};
      ...
      \path(ctrl_d)+(\nodesep,0) node (ctrl_d_gain) [simple_block] {$D_{\Omega_j}$};
      \path(ref_sum)+(3*\nodesep,1.5*\blockheight) node (ctrl_p_gain) [simple_block] {$P_{\Omega_j}$};
      ...
      %===================================================================================
      \path[draw,->] (ref.east) -- node[anchor=south, pos=0] {$\Omega_{j_{sp}}$} (ref_sum.west);
  
      \path[draw,->] (ref_sum.east) -- (ctrl_i.west);
    %   \path[draw,->] (ctrl_k.east) -- (ctrl_i.west);
      \path[draw,->] (output.east) |- node[anchor=south, pos=0] {$\Omega_j$} (ctrl_d_filt.west);
    %   \path[draw,->] (ctrl_kd.east) -- (ctrl_d_filt.west);
      ...
      \path[draw,->] (sat_ctrl.east) -- node[anchor=south] {$\delta_{j_{sp}}$} (ctrl.west);
    
    \end{tikzpicture}

    \caption{Angular rate controller diagram \cite{PX4_controller_diagrams}}
