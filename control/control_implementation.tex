...
\label{sec:control_implementation}
    ...
    to the multirotor-payload system.
    The control architectures are summarised in Table~\ref{tbl:controller_summary} 
    by pairing the system identification techniques with the appropriate controllers.
    In this work, the \emph{\gls{MPC} architecture} refers to the entire control implementation, 
    which includes the data-driven system identification method and the \gls{MPC}.
    Likewise, the \emph{\gls{LQR} architecture} includes the white-box modelling, the parameter estimation methods, and the resulting \gls{LQR} determined from the system identification model.
    These control architectures will be tested in simulation and their results will be shown and compared in this section.
    
    % It was firstly shown in Chapter~\ref{chap:system_id} that the system identification techniques worked in simulation.
    ...
    \FloatBarrier\subsection{Simple suspended payload} \label{sec:simple_payload_control}
        ...
        This payload model is also used for simulations with an \gls{LQR} controller in other studies \cite{Erasmus2020, Slabber2020}.
        ...
        This is probably due to the inherent similarity between the controller implementations as discussed in Section~\ref{sec:mpc}.
        The similarity of the acceleration setpoint data also shows that the energy expended in a velocity step is roughly equal for these two controller implementations.
        However, this is also highly dependant on the weightings used in the optimisation problem of both controllers.
        Both controllers also produce a non-zero steady-state setpoint as expected, which is required to counter aerodynamic drag.
    ...
    \FloatBarrier\subsection{Different payload parameters}
        ...
        Therefore, numerous flights with a range of different payloads were simulated, the respective models were trained and the controllers were implemented.
        Figure~\ref{fig:compare_control_vel_l1} and Figure~\ref{fig:compare_control_vel_l05} show velocity step responses with \gls{LQR} and \gls{MPC} implementations with two payloads flights.
        ...
        This shows that each control architecture is adaptable to different payload parameters without manual intervention.
    ...
    \FloatBarrier\subsection{Dynamic payload}
        ...
        This model accurately predicted the multirotor-payload dynamics, including the low and high-frequency dynamics, of a set of testing data.
        ...
        \paragraph
        Figure~\ref{fig:compare_control_theta_dynamic} shows the cable angle for a velocity step response.
        ...
        However, the actual dynamics do not follow the optimised trajectory of the \gls{MPC} and the \gls{MPC} does not conclusively outperform the \gls{LQR} implementation.
    ...
    \FloatBarrier\subsection{Change in unconsidered system parameters}
        ...
        \paragraph
        The control architectures were shown to be adaptable to variations in the payload parameters.
        ...
        Hence, changing the mass of the multirotor is detrimental to the accuracy of the plant model and therefore affects the \gls{LQR} performance.
        ...
        This is expected since the system identification model used by the \gls{MPC} included the effect of the changed mass by estimating the entire model without considering individual parameters.
...
\FloatBarrier\section{Summary}
    ...
    \paragraph
    From simulations without wind disturbances, the \gls{MPC} and \gls{LQR} architectures deliver similar control performances for a range of different payload parameters.
    ...
    However, the trajectories were not as smooth as with the simply suspended payload.
    ...
    Therefore, the parameter estimation procedure will need to be redesigned to account for such changes in other system parameters.
