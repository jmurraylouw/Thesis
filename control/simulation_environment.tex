\FloatBarrier\section{Simulation Environment} \label{sec:simulation_environment}
    ...
    This is used rather than the \gls{SITL} and Gazebo simulation environment because it allows us to iterate designs faster.
    ...
    In contrast, the \gls{SITL} and Gazebo simulation environment requires text-based control laws with C\texttt{++} and requires \gls{ROS} nodes to interface an \gls{MPC} with PX4.
    This requires a lot more development time for control system design than the graphical tools in Simulink.
    The \gls{SITL} and Gazebo simulation also has a longer runtime per simulation, which further adds to the development time of the iterative control system design process.
