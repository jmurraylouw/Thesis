...
% ?? remove all MV and MO
    \paragraph
    Shows the structure of a typical \gls{MPC} implemnetation.
    ...
    \FloatBarrier\subsection{Receding horizon}
        ...
        \gls{MPC} is also refered to as Receding Horizon Control, because the control sequence is determined for the next prediction horizon period at every time-step.
        ...
        % Therefore the \gls{MPC} determines a control sequence that would produce the optimsed trajectory of the output variable.
        ...
        \end{equation}
        which minimizes a specific objective function over the control horizon.
        The controller decision produces the predicted trajectory when applied to the plant model.
        Note that only the control action, $\bm{u}( k | k )$, will be executed and a new controller decsion will be determined at time-step $k+1$.
    ...
    \FloatBarrier\subsection{Plant model} \label{sec:mpc_model}
        ...
        rather than $\theta$.
        This is because $\theta$ has a non-zero steady-state value due to aerodynamic drag during a velocity step response.
    ...
    \FloatBarrier\subsection{Algorithm implementation}
        ...
        \paragraph
        There are numerous methods available to implement an \gls{MPC} for the use case presented in this work.
        An extensive list of these options is provided in the survey by \cite{Nguyen2020}.
        ...
        \paragraph
        The Simulink implementaion of an \gls{MPC} from the Model Predictive Control Toolbox\texttrademark \cite{MPCtoolbox2019} was selected for this work. 
        This is because the implemenation can be applied with the structure of the data-driven state-space models,
        and it integrates well with our simulation environment in Simulink.
        The Model Predictive Control Toolbox\texttrademark also supports C++ code generation for stand-alone ROS nodes.
        Therefore, it can integrate with the SITL implementation of PX4, and also be executed on a companion computer for a practical implementation.
        Therefore the selected MPC implementation appears to be the 

        % ?? Check all Simulink without TM
        
        \paragraph
        The Model Predictive Control Toolbox\texttrademark ~solves the control optimisattion problem as a \gls{QP} at each time interval \cite{MPCtoolbox2019}.
        It applies an active-set \gls{QP} solver using the KWIK algorithm from \cite{Schmid1994}.
        ...
        The controller desicion is the $\bm{u}_{mpc}$ adjustments determined by the \gls{QP} solver that minimise the objective function.
        ...
        \paragraph
        A powerful advantage of \gls{MPC} is that constraints are easily included into the optimal control law.
        However, constraints are not neccesary in this work and will not be applied.
        This is advantageous for practical implementations, 
        because unconstrained \gls{MPC} is less computationally complex than constrained \gls{MPC}.
        ...
        However, if $J_\Delta u (\bm{z}_k)$ is removed from the objective function, the \gls{MPC} and \gls{LQR} controllers optimise the same variables.
...
% ?? convert acronyms to \gls{}
    ...
    % For example of disturbacne observer \gls{MPC} with multirotor: https://arxiv.org/pdf/2011.11104.pdf
        ...
        % The \gls{MPC} optimiser will genertae a solution, $\bm{z}_k$, to accelerate slowly to the new velocity setpoint without disturbing the paylaod angle too much. 
...
% ?? Talk about design of value 0.1 in distrubance model
    ...
    \FloatBarrier\subsection{Tuning}
        ...
        The MPC is firstly tuned to have a similar responce time to the LQR so that the controller performacnes can be roughly compared.
        ...
        \paragraph
        In the controller tuning process, a large value of ${w^y}_{j}$ correspond to an aggresive maneuvers of the corresponding output variable, 
        because of the tracking error of that variable will be heavily penalised.
        In contrast, small values of ${w^u}_{j}$ or ${w^{\Delta u}}_{j}$, correspond to aggresive maneuvers, 
        ...
        \paragraph
        The objective function weights were iteratively tuned for a desired control performance in a similar way to the LQR.
        All weigths corresponding to the dealy-coordinates are set to zero.
        Due to the non-zero steady-state value of $\theta$, the corresponding weight is also set to zero.
        Hereafter, the value of $T_p = T_c$ is incrementally decreased untill a noticeable change in the controller performnace.
        $T_p$ is fixed at the smallest value before this change occurs.
        $T_c$ is then further decreased until a noticeable change in performnace.
        ...
        \begin{table}[!h]
            ...
            \begin{tabularx}{0.75\linewidth}{@{}cc@{}}
                \toprule
