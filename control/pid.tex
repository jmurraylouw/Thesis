\FloatBarrier\section{Cascaded PID control}
    ...
    \FloatBarrier\subsection{Angular rate controller}
        ...
        \paragraph
        Three separate linear \gls{PID} controllers are used to control angular rates in the pitch, roll, and yaw axis of the multirotor body frame.
        ...
        Some elements in the controller improve the practical control implementation, but have a negligible effect on the design on the gains, namely:
        \begin{itemize}
            \item A \gls{LPF} is added in the derivative path to reduce the effect of noise.
            \item The derivative path is implemented on the plant output signal, instead of the error signal to eliminate the derivative kick.
        ...
        % ?? Input figure of the angular rate step response
    ...
    \FloatBarrier\subsection{Angle controller}
        ...
        The proportional gain was designed for Honeybee by \cite{Grobler2020} for a faster transient response than with the default ZMR250 gain.
    ...
    \FloatBarrier\subsection{Translational controller}
        ...
        % ?? Ensure body frame and the inertial frame is explained in modelling chapter
        ...
        The velocity controller gains used for Honeybee by \cite{Grobler2020} result in aggressive velocity responses, which produces large payload swing angles.
        Such large payload angles are undesirable because this causes the vector subspace of the training data to be very different from the vector subspace the swing damping controllers will operate in.
        ...
        Note that the actual input of the plant is $A_{N_{sp}}$, however, it is assumed that $A_{N_{sp}} \approx A_N$ due to the large time-scale seperation between the velocity controller and attitude controller.
        ...
        \begin{align}\label{eq:lin_vel_quad_pl}
            ...
            G_{V_N}(s) &= \dfrac{s^2 + \frac{c_{\theta}}{m_p l^2}s + \dfrac{g}{l}}{s \left[s^2 + \dfrac{c_{\theta}(m_Q + m_p)}{m_Q m_p l^2}s + \dfrac{g(m_Q + m_p)}{m_Q l}\right]} 
        \end{align}
        ...
        The initial swing angle is quite large, however, this attenuates to lower angles quickly enough for stable flight.
        ...
        % ?? Check connection with system id chapter for the training data controller
