...
\label{chap:control}
    ...
    \FloatBarrier\section{Cascaded PID}
        ...
        \FloatBarrier\subsection{Translational controller}
            ...
            Position control is not required to produce training data in this work, therefore it will not be discussed in this section.
            ...
            The velocity controller diagram is shown in Figure~\ref{fig:velocity_controller}.

            \input{control/diagrams/veloicty_controller.tex}

            \paragraph
            The velocity controller gains used by \cite{Grobler2020} result in aggressive veloicty responses, which produces large payload swing angles.
            ...
            The system identification methods produce linear models, which are used in active swing damping controllers to minise the payload swing angles during flight.
            Therefore the models do not need to approximate  
            
            % ?? Check connection with system id chapter for training data controller

            The position controller applies a proportional term only
