...
\label{chap:control}
    ...
    \FloatBarrier\section{LQR} \label{sec:lqr}
        ...
        \end{equation}
        The augmented state space model used to deign the LQR is therefore given by:
        \begin{equation}
        ...
        \paragraph
        % Figure~\ref{fig:}
        shows the resulting controller digram.

        \paragraph
        The LQR control law is defined as:
        \begin{equation} \label{eg:lqr_control_law}
            \bm{u} = -\bm{K}_{lqr} \bm{x}_a ,
        \end{equation}
        where $\bm{K}_{lqr}$ is the LQR gain.
        Furthermore, the LQR considers the cost function:
        \begin{equation}\label{eq:lqr_cost_function}
            J( \bm{u} ) = \int_{0}^{ \infty } \left( \bm{X}_A^T \bm{Q} \bm{X}_A + \bm{u}^T \bm{R} \bm{u} \right) dt ,
        \end{equation}
