...
\label{chap:control}
    ...
    \FloatBarrier\section{Conclusion}
        ...
        \paragraph
        From the simulations with a simple suspended payload without wind disturbances, it was shown that the MPC and \gls{LQR} implementations deliver similar control performances for a range of different payload parameters.
        Both controllers result in a similar response time and velocity overshoot.
        The payload swing angle is also damped well by both controllers.
        ...
        For each of the different payloads, the system identification methods determined accurate models which were used for effective swing damping control.
        Even though the parameter estimation techniques (used with the LQR) do not consider the mass of the multirotor ($m_Q$),
        the \gls{LQR} architecture still provided acceptable swing damping control for small changes in $m_Q$.
        However, it was shown that for specific values of $m_Q$ the \gls{LQR} architecture produces an undesirable control response.
        The parameter estimation procedure will need to be redesigned to account for such changes in system parameters.
        However, the data-driven approach does not rely on modelling assumptions and the MPC effectively damped the payload oscillations for different values of $m_Q$.
        ...
        However, the \gls{LQR} implementations which are tuned for effective disturbance rejection result in a large velocity overshoot due to the integrator build-up.
        ...
        For both architectures, the velocity response showed high-frequency oscillations, unlike the smooth trajectories achieved with the simple suspended payload.
        ...
        \paragraph
        The MPC also managed to damp the payload oscillations, but the \gls{LQR} produced a smoother response than the MPC.
        The estimated model used by the \gls{MPC}, showed good prediction accuracy with the given testing data set, however, the actual response of the system the not follow the predicted trajectory of the MPC.
        ...
        Therefore, an improved data-driven system identification model, which provides an accurate approximation over a larger vector subspace, is required for effective MPC control of such a dynamic payload.

        \paragraph
        The advantage of the \gls{LQR} approach is computationally simple and could be implemented on onboard UAV hardware without a companion computer. 
        The \gls{LQR} control law applies a simple matrix multiplication for each time step.
        In contrast, an MPC performs a complex trajectory optimisation at every time step which may be too computationally complex for an onboard microcontroller.
        The \gls{LQR} also outperforms the MPC with a dynamic payload that differs significantly from its plant model, which shows that the \gls{LQR} is quite robust to model inaccuracies.
        Unlike the \gls{LQR} architecture, the data-driven system identification process of the MPC architecture does not require a manual redesign of the procedure to accommodate different unknown parameters.
        The advantage of the data-driven approach is that it provides a general solution for a range of different system configurations without an a priori modelling process.
        Unlike the white-box modelling approach, it handles systems with different system parameters without individually estimating each parameter value.
