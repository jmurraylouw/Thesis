...
\label{chap:results}
    ...
    \FloatBarrier\section{Parameter estimation with practical data}
        ...
        \FloatBarrier\subsection{Simple payload cable length estimation}
            ...
            \gls{PE} can be interpretted as the percentage of the actual length by which the estimated length differs from the actual length.
            ...
            Hence, an incorrect length is estimated from the frequency peak identified in the \gls{FFT}, but this length still corresponds to the dominant oscillation frequency.
            
            % \input{results/plots/prediction_single_pend_white_prac.tex}

            Figure~\ref{fig:prediction_single_pend_white_prac} compares the actual payload angle to the predicted payload angle of the white-box model for a practical flight. 
            The cable length estimation from the data in this plot has a \gls{PE} of 34.2~\%.
            ...
            However, only a few different wind speeds are considered and a trend cannot be identified conclusively from this small sample.

            % \input{results/plots/white_prediction_single_pend_wind.tex}

            \paragraph
            Note that the estimation \gls{PE} converges for each considered flight with a different wind speed,
        ...
        \FloatBarrier\subsection{Dynamic payload cable length estimation}
            ...
            As the size of the payload swing angles decreases due to damping, 
            the relative oscillations of the elongated payload also decrease.
            Therefore the effect of the superimposed higher frequency oscillations are less prominent and the system dynamics approximate a simple pendulum more closely.
