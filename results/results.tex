...
\chapter{Practical implementation and results} \label{chap:results}
    ...
    \paragraph
    In Chapter~\ref{chap:system_id}, it was shown within simulation that both the white-box and black-box system identification models could
    accurately represent the dynamics of a multirotor with a suspended payload.
    In Chapter~\ref{chap:control}, it was also shown within simulation that the proposed controllers effectively achieve swing-damping control. 
    ...
    \FloatBarrier\section{Parameter estimation with practical data}
        ...
        \paragraph
        In Section~\ref{sec:param_estimation}, a cable length parameter estimation technique was performed with simulation data and the resulting white-box models provided reasonably accurate representations of the simulated quadrotor-payload dynamics.
        ...
        \FloatBarrier\subsection{Simple payload cable length estimation}
            ...
            \paragraph 
            Figure~\ref{fig:cable_length_vs_train_time} shows the \gls{PE} of the cable length estimation for practical flight data with different payload masses and cable lengths.
            ...
            Unlike in the simulations, the actual cable attachement may be below the \gls{CoM} of the vehicle.
            ...
            However, the transient response of the practical data, can be seen in the first two oscillation peaks, is noticeably different from the model prediction.
            ...
            \paragraph
            Furthermore, the practical swing angle peaks are clearly attenuated by non-linear damping, which differs from the linear damping of the white-box model.
            ...
            It appears that the wind speed does affect the parameter estimation result since the estimation error differs significantly for different wind speeds.
    ...
    \FloatBarrier\section{Data-driven system identification with practical data} \label{sec:data_driven_sys_id_prac}
        ...
        \FloatBarrier\subsection{Wind disturbance} \label{sec:length_train_data_prac}
            ...
            The \gls{DMDc} implementation is therefore preferred over \gls{HAVOKc} for practical data due to lower computational complexity.
    ...
    \FloatBarrier\section{HITL}
        ...
        % ?? see that exp design chapter links with this. 
        % ?? Check all "as described in refs"
        \paragraph
        ...
        Firstly, the effect of the plant model size on the \gls{CPU} and \gls{RAM} consumption of the \gls{MPC} node will be investigated.
        ...
        \FloatBarrier\subsection{Effect of hyperparameters on computational requirements}    
            ...
            \paragraph
            As discussed in Section~\ref{sec:mpc}, the computational complexity of the \gls{MPC} optimisation problem increases for larger state-space matrices, and the hyper parameter, $q$, determines the size of these matrices.
            Figure~\ref{fig:CPU_vs_q} shows the \%CPU consumption of \gls{MPC} nodes based on models with different $q$ values.
            The \%CPU value represents the average percentage of \gls{CPU} time used by the \gls{MPC} node running on the companion computer.
            ...
            Figure~\ref{fig:CPU_vs_q} shows that the companion computer runs the \gls{MPC} node with $q = 50$ and $T_s =$~\SI{0.03}{\second} at \%CPU~$= 82.8\%$.
            ...
            The \gls{MPC} node achieved stable, swing-damping control with this model.
            However, for $q = 70$ the \gls{MPC} node used \%CPU~$= 98.7\%$ and the optimisation problem could not be solved fast enough for stable control.
            This resulted in unstable control and the quadrotor-payload system crashed consistently with this \gls{MPC} node. 

            \input{results/plots/RAM_vs_q.tex}

            \paragraph
            Figure~\ref{fig:RAM_vs_q} shows that the \%RAM also increases for larger values of $q$.
            The \%RAM value represents the maximum percentage of \gls{RAM} space used by the \gls{MPC} node while running on the companion computer.
            It is clear that the companion computer has sufficient memory to handle the \gls{MPC} node memory requirements, since the \gls{MPC} uses less than 1\% RAM, even with $q = 70$.
        ...
        \FloatBarrier\subsection{Effect of sample time on computational requirements}    
            ...
            \paragraph
            The sample time of the controller also affects the \%CPU consumption of the \gls{MPC} node.
            Figure~\ref{fig:CPU_vs_Ts} shows the measured \%CPU of \gls{MPC} nodes with different sample times using a plant model with $q = 50$. 
            ...
            The standard \gls{PID} velocity controller in PX4 runs at \SI{50}{\hertz}, which corresponds to $T_s =$~\SI{20}{\milli\second}.
            However, an \gls{MPC} node with $T_s =$~\SI{20}{\milli\second} struggles to run fast enough on the given companion computer.
            This \gls{MPC} node resulted in unstable control because the QP problem cannot be solved within the given optimisation time.
            
            \paragraph
            A \gls{MPC} node with $T_s =$~\SI{40}{\milli\second} runs with a low \%CPU, however it also results in unstable control.
            ...
            A \gls{MPC} node with $T_s =$~\SI{30}{\milli\second} provides stable control of the quadrotor-payload system.
        ...
        \FloatBarrier\subsection{Velocity step response}    
            ...
            \paragraph
            Figure~\ref{fig:HITL_mpc_vel} plots the velocity responses of the \gls{PID} and \gls{MPC} controllers in \gls{HITL} simulations of the qaudrotor and suspended payload system.
            The \gls{MPC} controller is based on a system identification model with $q = 50$ and has a sample time of $T_s =$~\SI{30}{\milli\second}.
            It is clear from this plot that the \gls{MPC} node running on the companion computer provides stable control of the quadrotor payload system and damps the velocity oscillations well.
            ...
            From this plot it is clearly visible that the \gls{MPC} controller damps the payload angle well.
            Overall, these plots show that the swing-damping control performance shown in simulation in Section~\ref{sec:mpc}, is also achievable with the final controller software running on the actual hardware.

            % ?? Discuss practical feasibility as in intro

    \FloatBarrier\section{Summary}

        \paragraph
        In this chapter, the system identification techniques were applied to practical flight data.
        Firstly, the methodology used to obtain the flight data and apply the techniques were described.
        ...
        It was shown that the estimated length converged quickly for each payload configuration and that the estimation error was consistently quite large.
        
        \paragraph
        It was also shown that the estimated length converged for flight data with different wind conditions, and it that the estimation error was the largest for the data with the most wind.
        Furthermore, the cable length estimation technique was applied to flight data with a double-pendulum payload.
        The estimated value converged to a length corresponding to the dominant frequency in the data, and the resulting white-box model prediction captured the general shape of the data well.
        However, the white-box model prediction was shown to be very sensitive to initial conditions.

        \paragraph
        The data-driven system identification techniques were also successfully applied to practical flight data.
        As expected, the prediction error of the techniques were decrease for flights with lower wind speeds.
        Even with a considerable amount of wind, the techniques were able to generate accurate prediction models.
        However at very high wind speeds, the resultant models were unusable.

        \paragraph
        The \gls{DMDc} and \gls{HAVOKc} state predictions performed equally well on data from various different flights.
        \gls{DMDc} was chosen as the preferred method due to its lower complexity.
        It was shown that \gls{DMDc} could be applied to data from a range of different payload parameters and consistently generate accurate prediction models without prior knowledge of the payload configuration.
        The Pareto front and `double-descent' trends discussed in Chapter~\ref{chap:system_id} were also identified in these results.
        
        \paragraph
        \gls{DMDc} was also extended to capture the North and East velocity dynamics simultaneously, and the resulting prediction model accurately reconstructed the system dynamics for test data.
        Furthermore, \gls{DMDc} was applied to flight data with a dynamic payload representing a double-pendulum system.
        \gls{DMDc} also accurately reconstructed the dynamics of this system for unseen test data.
        ...
        It was shown that the hardware successfully executes the \gls{MPC} algorithm at the desired speed for the selected hyperparameters and sample time.
        The velocity step response proved that the various software systems work together seamlessly.
        It also showed that the control architecture can be implemented on the hardware of an actual multirotor to successfully apply swing-damping control for the quadrotor-payload system.
            
        % ?? Is hierdie te veel detail? Moet ek dit korter maak?
}