...
\chapter{Practical implementation and results} \label{chap:results}
    ...
    Furthermore, \gls{HITL} simulations will be performed to determine whether the available hardware can handle the computational complexity of the MPC implementation.
    ...
    \FloatBarrier\section{HITL}
        ...
        \FloatBarrier\subsection{Velocity step response}    
            ...
            % ?? Discuss practical feasiblity as in intro
    ...
    \FloatBarrier\section{Summary}
        ...
        \paragraph
        The DMDc and HAVOKc state predictions performed equally well on data from various different flights.
        DMDc was chosen as the preferred method due to its lower complexity.
        It was shown that DMDc could be applied to data from a range of different payload parameters and consistently generate accurate prediction models without prior knowledge of the payload configuration.
        The Pareto front and `double-descent' trends discussed in Chapter~\ref{chap:system_id} were also identified in these results.
        
        \paragraph
        DMDc was also extended to capture the North and East velocity dynamics simultaneously, and the resulting prediction model accurately reconstructed the system dynamics for test data.
        Furthermore, DMDc was applied to flight data with a dynamic payload representing a double-pendulum system.
        DMDc also accurately reconstructed the dynamics of this system for unseen test data.
        Unlike the white-box model which only represented the dominant oscillation frequency, the DMDc model also predicted the irregular oscillations well.

        \paragraph
        Finally, the full data-driven system identification with MPC control architecture was tested in HITL simulations.
        It was shown that the hardware successfully executes the MPC algorithm at the desired speed for the selected hyperparameters and sample time.
