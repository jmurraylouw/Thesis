...
\label{chap:results}
    ...
    \FloatBarrier\section{HITL}
        ...
        The \gls{FC} runs the actual PX4 flight stack and executes the control signals received from the \gls{MPC} node.
        ...
        The \%CPU value represents the average percentage of CPU time used by the MPC node running on the companion computer.
        Explain \%CPU

        \input{results/plots/RAM_vs_q.tex}

        \paragraph
        Figure~\ref{fig:RAM_vs_q}
        Explain \%RAM

        \input{results/plots/CPU_vs_Ts.tex}

        \paragraph
        Figure~\ref{fig:CPU_vs_Ts}
        Why 3?

        Never could not find viable solution. When would it not find a solution?

        \input{results/plots/HITL_mpc_vel.tex}

        \paragraph
        Figure~\ref{fig:HITL_mpc_vel}

        \input{results/plots/HITL_mpc_theta.tex}

        \paragraph
        Figure~\ref{fig:HITL_mpc_theta}
