...
\label{chap:results}
    ...
    \paragraph
    In previous chapters it was shown from simulation data that both the white-box and black-box system identifcation techniques
    generated models that accurately capture the dynamics of a multirotor with a suspended payload.
    However, practical flights may differ significantly from simulations, which may affect the performance of these techniques.
    Wind causes a complex, unmeasured disturbance which influences the flight of a multirotor, 
    but was not considered during simulation.
    Wind applies randomly fluctuating forces to the vehicle, payload and cable which and can be difficult to model accurately.
    It is also very difficult to determine accurate drag coefficients of the practical system.

    \paragraph
    In this chapter the system identification techniques will be applied to practical flight data and their performance will be evaluated.
    Practical flight data with a dynamic payload will also used 
    to compare the performance of white-box and black-box techniques on this use case.
    Furthermore it will be investigated whether the models produced by these techniques
    can be used in the proposed controllers to effectively damp the payload oscillations in simulation.
    The performance of the controllers will be evaluated for a range of different system configurations.
    HITL simualtions will also be performed to determine whether the proposed hardware 
    can handle the computational complexity of the controllers.
    ...
    \FloatBarrier\section{Methodology}
        ...
        Firstly, the mulitrotor hovers with the suspended payload to gather hover thrust data to estimate the payload mass.
        Then the mulitrotor is commanded with a position step setpoint 
    ...
    \FloatBarrier\section{Parameter estimation with practical data}
        ...
        \FloatBarrier\subsection{Simple payload cable length estimation}
            ...
            \paragraph
            The \gls{PE} for the cable length estimation is calculated as,
            \begin{equation}
