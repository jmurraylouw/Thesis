...
\label{chap:results}
    ...
    \FloatBarrier\section{Parameter estimation with practical data}
        ...
        \FloatBarrier\subsection{Dynamic payload cable length estimation}
            ...
            from the resulting FFT amplitude spectrum in Figure~\ref{fig:FFT_double_pend_prac}.
    ...
    \FloatBarrier\section{Data-driven system identification with practical data}
        ...
        \paragraph
        In Chapter~\ref{chap:system_id} it was shown that \gls{DMDc} and \gls{HAVOK} 
        build accurate state prediction models for the considered system in simulation.
        It was also shown in Chapter~\ref{chap:modelling} that the simulation environment 
        is a realistic representation of the practical system.
        However, there are still differences between simulations and practical flight.
        Therefore the data-driven algorithms will be applied to practical flight data in this chapter 
        to investigate their performance in a practical implementation.

        \FloatBarrier\subsection{Length of training data} \label{sec:length_train_data_prac}

            \paragraph
            As discussed in Section~\ref{sec:sys_id_length_of_data}, 
            the length of training data used in a model has a large effect on prediction accuracy.
            ...
            \paragraph
            Wind adds an unmeasured disturbance (also referred to as process noise) to the plant which is not accounted for in the model.
            This disturbance consists of a mean force and random fluctuations in the force, 
            which are applied to the vehicle, cable and payload.
            The mean force applied to the multirotor 
            results in a constant offset in the acceleration setpoint data 
            as the velocity controller integrators compensates for the disturbance.
            For system identification, the constant offset is subtracted from the acceleration setpoint data, 
            ...
            However, the wind speed also deviates randomly from the mean speed.
            This results in random process noise in the plant which is not removed from the measured data.
            ...
            \paragraph
            Figure~\ref{fig:havok_vs_dmd_Ttrain_2mps} compares the prediction error of \gls{DMDc} and \gls{HAVOK} models 
            as a function of length of training data.
            It is evident that the two techniques produce similar prediction errors with practical data.
            In Chapter~\ref{chap:system_id} it was shown with simulation data 
            that \gls{HAVOK} produces slightly lower prediction errors thatn \gls{DMDc}.
            Figure~\ref{fig:havok_vs_dmd_Ttrain_2mps} comfirms this observation for practical data.
            However, this error difference is very small 
            ...
            Also plot \gls{DMDc} vs \gls{HAVOK} for high winds ??
        ...
        \FloatBarrier\subsection{System parameters}
            ...
            Hence the effect of system paramaters such as 
            multirotor mass, 
            payload mass, 
            cable length, 
        ...
        \FloatBarrier\subsection{State predictions}
            ...
            Note that the model was generated from training data but is tested with a separate set of previously unseen, testing data.
            Figure~\ref{fig:prac_pediction_single_pend_theta_black} plots the state prediction against testing data.
            For each prediction run, the model starts with an initial condition, $\bm{x}_{0}$.
            The state vector of each future time-step, $\bm{x}_{k+1}$, 
            is propogated from the previous state vector, $\bm{x}_{k}$, 
            and the input vector logged for that time-step, $\bm{u}_{k}$.
            Each of these time-step predictions results in a small error which accumulates as the prediction horizon increases.
            Therefore the prediction error increases as time progresses, as shown in Figure~\ref{fig:prac_pediction_single_pend_theta_black}.
        ...
        \FloatBarrier\subsection{Extended dimensions}
            ...
            Figure~\ref{fig:xy_predictions} \gls{DMDc} predictions
