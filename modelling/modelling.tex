...
\paragraph
In this chapter, a non-linear mathematical model will be derived of a multirotor with a suspended payload.
This model will be used to simulate the multirotor-payload system in subsequent chapters.
The models will be based on the real quadrotor named \emph{Honeybee}, which was built by \cite{Grobler2020} and will be described further in Chapter~\ref{chap:exp_design}.

\paragraph
The chapter starts by defining the coordinate frames and rotation representations used in the model.
A \gls{6DOF} model is then derived of the quadrotor vehicle and the different forces and moments acting on this vehicle are described.
A model of the suspended payload attached to the quadrotor is then derived.
Finally the mathematical models are verified against practical flight data from \emph{Honeybee}.
...
\FloatBarrier\section{Suspended payload model} \label{sec:payload_model}
    ...
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.65\linewidth]{modelling/fig/quad_with_payload.png}
        \caption{Schematic of a quadrotor with suspended payload from \cite{Slabber2020}}
        \label{fig:quad_with_payload}
    \end{figure}
    
    \FloatBarrier\subsection{Payload assumptions} \label{sec:payload_assumptions}
    ...
    \FloatBarrier\subsection{Non-conservative forces}
        ...
        \paragraph
        The non-conservative forces and moments acting on the payload are also considered in this derivation.
        This refers to the effects that add or remove energy from the system.
        The non-conservative forces and moments include
        the aerodynamic drag force acting on the payload and the moment caused by the friction of the cable attachment.
        ...
        \begin{align}
            M_{\theta}^F  &= - c_{\theta} \dot{\theta} \\
            M_{\phi}^F    &= - c_{\theta} \dot{\phi}
        \end{align}
        respectively, where $c_{\theta}$ is the rotational friction coefficient for $\theta$ and $\phi$ rotations.
...
\FloatBarrier\section{Model verification} \label{sec:model_verification}
    ...
    This flight was also performed on a day with minimal wind to try exclude the effect of wind disturbances in the comparison.
    
    \paragraph
    Starting from the same initial condition, the same velocity step setpoint was commanded in simulation, and the resulting simulation data was compared to the practical flight data.
    ...
    In this figure the velocity response do not correlate as well as they do for the experiment without a payload.
    ...
    This seems to be an impressive result for such a complex system without so may different elements.
    Both systems provide a overshoot of a similar value.
