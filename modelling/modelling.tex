...
\FloatBarrier\section{Forces and moments}
    ...
    The suspended payload applies a reaction force and moment to the quadrotor which is dependant on the equations of motion of the suspended payload.
...
\FloatBarrier\section{Suspended payload model}
    ...
    The equations of motion of the suspended payload need to be derived to define these reaction forces.
    The considered payload model and modelling assumptions will first be discussed in this section.
    Thereafter, the payload equations of motions will be derived with Lagrangian mechanics.
    Finally, the resultant forces of the payload on the quadrotor will be presented.

    \paragraph
    Figure~\ref{fig:quad_payload} illustrates a quadrotor with a suspended payload, where
    $m_Q$ is the mass of the quadrotor,
    ...
    \FloatBarrier\subsection{Lagrangian}
        ...
        The dynamical equations of the same suspended payload system were derived by \cite{Slabber2020} and the derivation in this work closely follows that approach.
        ...
        The vector of generalised coordinates, $\bm{p}$, of the whole system can now be defined as,
        ...
        \paragraph
        The total kinetic energy, $T_e$, and total potential energy, $V_e$, of the quadrotor and payload system can be determined as,
        \begin{align}
            T_e &= \frac{1}{2} m_Q | \bm{\dot{r}}_Q | ^2 + \frac{1}{2} m_p | \bm{\dot{r}}_p | ^2, \mbox{~~and~~} \\
            V_e &= - m_Q g z_Q - m_p g z_p .
        \end{align}
    ...
    \FloatBarrier\subsection{Non-conservative forces}
        ...
        \begin{align}
            ...
            \end{bmatrix} = 
            \begin{bmatrix}
                \frac{1}{2} \phantom{.} \rho \phantom{.} C_{Q_x} \phantom{.} \dot{x_Q}^2 \\
                \frac{1}{2} \phantom{.} \rho \phantom{.} C_{Q_y} \phantom{.} \dot{y_Q}^2 \\
                \frac{1}{2} \phantom{.} \rho \phantom{.} C_{Q_z} \phantom{.} \dot{z_Q}^2
            \end{bmatrix}  \mbox{ and}  \label{eq:quad_drag_inertial} \\
            \bm{F}_p^A & = 
            \begin{bmatrix}
                F_{p_x}^A \\
                F_{p_y}^A \\
        ...
        \begin{equation}
            \bm{Q} = 
            \begin{bmatrix*}[c]
                F_{\mathcal{I}_x} - F_{Q_x}^A - F_{p_x}^A \\
                F_{\mathcal{I}_y} - F_{Q_y}^A - F_{p_y}^A \\
                F_{\mathcal{I}_z} - F_{Q_z}^A - F_{p_z}^A \\
    ...
    \FloatBarrier\subsection{Quadrotor forces due to payload}
