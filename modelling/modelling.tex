...
The model will be based on the multirotor named \emph{Honeybee}, which was built by \cite{Grobler2020} and will be described further in Chapter~\ref{chap:exp_design}.
...
A \gls{6DOF} model of the multirotor vehicle will then be derived and the different forces and moments acting on this vehicle will be described.
A model of the suspended payload attached to the multirotor will also be derived.
...
\FloatBarrier\section{Coordinate frames}
    ...
    \paragraph
    Honeybee is quadrotor vehicle, which is a type of multirotor with four rotors.
    The quadrotor X-configuration is considered in this work.
    Figure~\ref{fig:coord_frames} shows the quadrotor schematic and two coordinate frames that will be used to describe this system.
    ...
    The inertial frame assumes a flat, non-rotating earth since the multirotor will travel small distances in comparison to the curvature of the earth.
...
\FloatBarrier\section{Quadrotor model}
    ...
    \paragraph
    The multirotor is modelled as a rigid body with six degrees of freedom.
    ...
    These parameters will be used in subsequent sections to derive a model of the multirotor.
    ...
    \paragraph
    The linear velocity and angular velocity of the multirotor with respect to the body frame is denoted by,
    \begin{align}
        \bm{V_\mathcal{B}}          &= 
        \begin{bmatrix}
    ...
    The forces and moments acting on the multirotor will be described in the next section.
...
\FloatBarrier\section{Suspended payload model} \label{sec:payload_model}
    ...
    Finally, the forces acting on the multirotor due to the payload will be determined.
    ...
    \subsection{Payload assumptions} \label{sec:payload_assumptions}
        ...
        They are attached with a low-friction swivel to the suspended cable, therefore the rotation of the payload around the cable axis has a negligible effect on the multirotor. 
    ...
    \FloatBarrier\subsection{Lagrangian}
        ...
        \end{align}
        Note that these energy equations describe the system modelled as two point-mass bodies and do not consider the attitude dynamics of the multirotor as mentioned earlier.
    ...
    \FloatBarrier\subsection{Payload forces acting on the multirotor}
        ...
        \paragraph
        This completes the multirotor dynamical model described in Section~\ref{sec:payload_model}.
...
\FloatBarrier\section{Model verification} \label{sec:model_verification}
    ...
    The practical multirotor will be described in Chapter~\ref{chap:exp_design}.
    ...
    Note that this includes the dynamics of the default multirotor controllers which will be described in Chapter~\ref{chap:control}.
    Therefore the results in this section compare the combined multirotor and controller simulation model to the practical flight data.
    ...
    Overall, the results in this section show that the simulation model provides a good representation of the North velocity dynamics of the real multirotor-payload system.
