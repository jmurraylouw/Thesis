...
This model will be used to simulate the multirotor-payload system in subsequent chapters.
The model will be based on the real quadrotor named \emph{Honeybee}, which was built by \cite{Grobler2020} and will be described further in Chapter~\ref{chap:exp_design}.

\paragraph
The chapter starts by defining the coordinate frames and rotation representations used in this work model.
A \gls{6DOF} model of the quadrotor vehicle will then be derived and the different forces and moments acting on this vehicle will be described.
A model of the suspended payload attached to the quadrotor will also be derived.
Finally, the mathematical models will be verified against practical flight data from \emph{Honeybee}.
...
\FloatBarrier\section{Coordinate frames}
    ...
    The origin of this frame is at the \gls{CoM} of the vehicle and the $x$, $y$, and $z$ axes, align with the forwards, rightwards, and downwards directions of the quadrotor body respectively.
    ...
    The mathematical representation of rotations will be discussed in the section below.
...
\FloatBarrier\section{Rotations}
    ...
    The attitude will be defined in terms of Euler angles or quaternions in this work, which will be described below.
    ...
    \FloatBarrier\subsection{Euler angles}
        ...
        Note that $\Theta$ and $\Phi$ are each illustrated here as a pure pitch and roll angle from the inertial frame, without a prior Euler rotation.
...
\FloatBarrier\section{Quadrotor model}
    ...
    This modelling process is well described by \cite{Erasmus2020} and \cite{Slabber2020}, and the same general procedure is followed in this work. 
    ...
    \paragraph
    This provides a set of \glspl{ODE} which fully describe the quadrotor motion in \gls{6DOF}, given the forces and moments acting on the vehicle.
...
\FloatBarrier\section{Forces and moments}
    ...
    \paragraph
    Different phenomena apply forces and moments to the quadrotor during flight.
    The total force and moment acting on the quadrotor in the body frame are given by,
    \begin{align}
    ...
    \begin{equation} \label{eq:thurst_force_moment}
        ...
        \end{bmatrix} ,
    \end{equation}
    where $d$ is the distance from each motor to the quadrotor \gls{CoM} and $R_N$ is the virtual yaw moment arm which is a property of the motor-propeller configuration.
    ...
    It is assumed that the moments caused by aerodynamics, such that,
    \begin{equation}
        \bm{M}_\mathcal{B}^A =
        \begin{bmatrix}
            0 \\
    ...
    Therefore the total force and moment acting on the quadrotor due to gravity is,
    \begin{equation}
    ...
    The suspended payload applies a reaction force, $\bm{F}_\mathcal{B}^P$, and moment, $\bm{M}_\mathcal{B}^P$, to the quadrotor which is dependant on the dynamics of the suspended payload.
...
\FloatBarrier\section{Suspended payload model} \label{sec:payload_model}
    ...
    The suspended payload can be modelled as a rigid body that is attached to the quadrotor by a rigid link.
    ...
    \paragraph
    Figure~\ref{fig:quad_with_payload} illustrates a quadrotor with a suspended payload, where
    ...
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.7\linewidth]{modelling/fig/quad_with_payload.png}
    ...
    \subsection{Payload assumptions} \label{sec:payload_assumptions}
        ...
        Due to this assumption, the payload cannot apply a moment to the quadrotor, hence the quadrotor attitude dynamics is decoupled from the payload dynamics.
    ...
    \FloatBarrier\subsection{Lagrangian}
        ...
        The dynamical equations of the suspended payload system were derived by \cite{Slabber2020} and the derivation in this work follows parts of that approach.
    ...
    \FloatBarrier\subsection{Equations of motion}
        ...
        \end{equation}
        which describes the motion of the payload as a function of the motion of the quadrotor \gls{CoM} in the inertial frame.
    ...
    \FloatBarrier\subsection{Payload forces acting on the quadrotor}
        ...
        % \end{equation}
        % where 
        % where describes the component of the acceleration of the quadrotor as a function of the motion
        The reaction force applied to the quadrotor due to the payload can now be determined in the inertial frame as,
        \begin{equation}
            \bm{F}_{\mathcal{I}}^P = m_Q \ddot{\bm{r}}_Q ,
        \end{equation}
        from Newton's second law, where $\ddot{\bm{r}}_Q$ represents the component of the quadrotor acceleration caused by the suspended payload.
        ...
        \paragraph
        This completes the quadrotor dynamical model described in Section~\ref{sec:payload_model}.
        The set of \glspl{ODE} represented by Equation~\ref{eq:payload_odes} has coupled the quadrotor and payload dynamics.
        The quadrotor-payload system can now be simulated from a given initial condition with the \gls{ODE} solver in Simulink\texttrademark.
        This simulation model will be verified with practical data in a subsequent section. 
    
    % \FloatBarrier\section{Linearised model} \label{sec:linear_model}
        
    %     \paragraph
    %     The dynamical equations derived in previous sections present a non-linear model of the quadrotor-payload system.
    ...
    %     The LQR controller and linear model derived by \cite{Slabber2020} will be discussed in this section and used in this work.
    %     This controller is used for North velocity control of a quadrotor with a suspended payload.
...
\FloatBarrier\section{Model verification} \label{sec:model_verification}
    ...
    Firstly, a North velocity step response was performed in a practical flight with a quadrotor to verify the quadrotor model without a payload.
    The practical quadrotor will be described in Chapter~\ref{chap:exp_design}.
    For simplicity in comparison, the body $\bar{\bm{x}}_{\mathcal{B}}$ axis was aligned with the inertial North axis for this flight.
    This flight was also performed on a day with almost no wind to try to exclude the effect of wind disturbances in the comparison.
    
    \paragraph
    Starting from the same initial condition, the same velocity step setpoint was commanded in a simulation, and the resulting simulation data was compared to the practical flight data.
    Note that this includes the dynamics of the default quadrotor controllers which will be described in Chapter~\ref{chap:control}.
    The Simulink\texttrademark~simulation environment will also be discussed in Chapter~\ref{chap:control}.
    The results in this section, therefore, compare the combined quadrotor and controller simulation model to practical flight data.

    \input{modelling/plots/prac_vs_sim_vel_step_no_payload.tex}

    \paragraph
    Figure~\ref{fig:prac_vs_sim_vel_step_no_payload} shows the velocity step response of a practical and simulated flight.
    From a visual inspection of the plots, it is clear that the velocity responses match well.
    Note that the practical velocity curve is more irregular and not as smooth as the simulated curve.
    This may be due to slight wind disturbances or measurement noise, which is not considered in this simulation.

    \input{modelling/plots/prac_vs_sim_vel_step_with_payload.tex}

    \paragraph
    The same procedure was followed to verify the quadrotor with a suspended payload model.
    ...
    In this figure, the velocity responses do not correlate as well as in the no-load experiment.
    This may be due to the added complexity of the model.
    The considered payload model adds a lot of modelling assumptions which add to the inaccuracy of the model.
    The system also considers more state variables that each need to be assigned an accurate initial condition value.
    Furthermore, the effect of wind disturbances is expected to be greater for the quadrotor-payload system than for the quadrotor without a load.

    \paragraph
    However, the shape of the practical and simulated velocity curves are still well matched.
    This seems to be an impressive result for such a complex system without many different elements.
    Both systems provide an overshoot of a similar value.
    The amplitude and frequency of the velocity oscillation are also visibly very similar.
    ...
    It is also clear that the payload angle response of the simulated system matches the practical data well. 
    Overall, the results in this section show that the simulation model provides a good representation of the real quadrotor-payload system.
    The mathematical model derived in this chapter will therefore be used for further simulations in subsequent chapters.
