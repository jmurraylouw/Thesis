...
\chapter{Experimental design} \label{chap:exp_design}
    ...
    \FloatBarrier\section{Hardware components}
        ...
        \paragraph
        The main hardware components in this work includes a multirotor vehicle, a payload angle sensor, and an \gls{OBC}.
        ...
        \FloatBarrier\subsection{Multirotor}
            ...
            \paragraph
            The physical parameters of this multirotor is summarised in Table~\ref{tbl:honeybee_specs}.
    ...
    \FloatBarrier\section{Software Toolchain}
        ...
        \FloatBarrier\subsection{QGroundControl}
    ...
    \FloatBarrier\section{Hardware-in-the-Loop simulations} \label{sec:exp_design_hitl}
        ...
        Figure~\ref{fig:HITL} illustrates how the different software and hardware components interlink for HITL simulations.
        
        \paragraph
        \gls{QGC} runs on the desktop computer and communicates with the Gazebo simulator with MAVlink via a local \gls{UDP} connection.
        ...
        The \gls{OBC} runs the MPC and MAVROS nodes which send and receive MAVLink messages to the \gls{FC} via a serial port connection.

        \begin{figure}[ht]
            \centering
            \includegraphics[width=0.9\linewidth]{exp_design/fig/HITL}
            \caption{Different software and hardware components of a HITL simulation}
            \label{fig:HITL}
        \end{figure}
    ...
    \FloatBarrier\section{Practical flights}
        ...
        \begin{figure}[!htb]
            \centering
            \includegraphics[width=0.6\linewidth]{honeybee_with_payload.jpg}
            \caption{Practical flight with Honeybee and a suspended payload}
            \label{fig:honeybee_with_payload}
        \end{figure}

        \paragraph
        Figure~\ref{fig:honeybee_with_payload} shows the Honeybee with a suspended payload during a practical flight.
