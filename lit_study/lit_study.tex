...
\section{Payload transportation with multirotors}
    ...
    \subsection{Rigid connection payloads}
        ...
        Often, the weight and size of the payload are unknown before a flight.
        ...
        However, this configuration limits the shape and size of a potential payload, since the payload needs to be compatible with the vehicle gripper.
    ...
    \subsection{Suspended payloads}
        ...
        The shape and mass of the payload affect the flight dynamics, but the payload is often unknown before a flight.
        ...
        \paragraph
        Various suspended payload configurations have been considered in the literature.
        ...
        Furthermore, the control of a group of multirotors cooperatively transporting a suspended payload has also been considered in various studies \cite{Lee2015, Sanalitro2020, Klausen2014, Goodarzi2015}.
        
        \paragraph % This paragraph seems out of place and does not flow well
        From numerous examples in the literature, it is clear that the control of multirotors with suspended payloads is a popular research topic.
...
\section{Control of multirotors with suspended payloads}
    ...
    \paragraph
    A major drawback of transporting a suspended payload is that the payload is free to swing during flight, which affects the dynamics of a multirotor.
    Two main control strategies are applied in the literature to stabilise a multirotor with a suspended payload, namely, trajectory generation and active vibration damping.
    ...
    \FloatBarrier\subsection{Swing damping controllers}
        ...
        The \gls{MPC} simulation results were also verified with experimental results in a practical indoor environment.
        ...
        \paragraph
        Other types of controllers have also been applied for active swing damping and will be discussed in Section~\ref{sec:review_swing_damping}.
    ...
    \FloatBarrier\section{Review of swing damping control studies} \label{sec:review_swing_damping}
        ...
        Table~\ref{tbl:lit} lists other works in the literature involving the active swing damping control of multirotors with suspended payloads.
        ...
        Baseline controllers are techniques that are considered well-known and are applied to a considered problem to compare the performance of a proposed technique to a reference.
        ...
        These studies can conclude that the proposed technique solves the considered problem, but it is not conclusive whether the technique improves on the performance of known controllers.
        ...
        Non-linear \gls{MPC} implementations that depend on a non-linear plant model have also been studied for the quadrotor with a suspended payload \cite{Trachte2014, Trachte2015}.
        ...
        However, some practical implementations apply controllers which do not depend on a plant model and show promising results \cite{Muthusamy2021, Hua2021, Faust2014}.
        ...
        Practical data may include sensor noise, modelling uncertainties, external disturbances, and other computational hardware effects like latency that are often not considered in simulations.
        ...
        Other studies designed controllers to account for parameter uncertainty, but only tested the tuned controller on a single payload case.
...
\section{Multirotor and suspended payload systems with unknown dynamics}
    ...
    Without prior knowledge of the system, the controller weights were autonomously tuned within \SI{30}{\second} of flight time to provide stable trajectory tracking of the multirotor.
    ...
    It was shown that trajectory tracking was significantly improved.
    ...
    These effects focus on counteracting the swinging payload disturbance as it happens and does not directly learn how to control the unknown payload.
...
\section{Summary} 
    ...
    \paragraph
    This chapter reviewed a range of different control solutions for a multirotor with a suspended payload.
    ...
    \begin{enumerate}
        ...
        \item Few studies consider some parameter uncertainty, but few studies consider unknown system dynamics.
        ...
        \item No studies were found in the literature that considered data-driven system identification of multirotors with suspended payloads.
    \end{enumerate}

    \paragraph
    The focus of this thesis will involve the stabilised control of a multirotor with an unknown suspended payload.
    The model uncertainty will not only include the parameter uncertainty but the dynamics of the suspended payload system will be considered unknown before a flight.
