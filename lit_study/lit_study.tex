...
\section{Control of a multirotor with a suspended payload}
    ...
    Two main control strategies are applied in literature to stabilise a multirotor with a suspended payload, namely, trajectory generation and active-damping control.
    Trajectory generation methods involve executing trajectories that induce minimal oscillations.
    Active-damping controllers involve feedback controllers that apply a control law to counteract the swing of the payload based on a measurement or estimate of the payload state.

    \subsection{Trajectory generation}

        \paragraph
        Trajectory generation methods for suspended payload systems are often based on open-loop control techniques.
        % A trajectory is calculated in which the multirotor motion induces as little oscillations as possible.
        Input shaping is a common open-loop control method for minimal swing control of suspended payloads.
        These techniques were originally designed for gantry systems with suspended payloads \cite{??}.
        Later these input shaping techniques were also applied for minimal swing control of helicopter \cite{??} and multirotor \cite{??} systems transporting suspended payloads.
        
        \paragraph
        Example of input shapers on multirotor system

        \paragraph
        The main trajectory generation techniques include, 
        MPC,
        input shapers,
        etc
        Name if simulation and practical

        Add cases from slabber and erasmus.
        Look for newer literature.

        \paragraph
        Slabber technique combo of input shape and active damping
