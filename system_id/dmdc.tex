...
\label{sec:dmdc}
    ...
    Note that this implementation is an adaptation of \gls{DMDc}, and includes time-delay-embedding of multiple observables. 
    ...
    However this implementation is not considered because a companion computer with significant memory size can be used.
    ...
    \end{equation}
    Note that the primary objective of \gls{DMDc} is to determine the best fit model matrices, $\bm{A}$, $\bm{A}_d$ and $\bm{B}$, 
    ...
    \end{equation}
    By reversing Equation~\ref{eq:G_Omega}, $\bm{G}$ can now be separated into:
    \(
