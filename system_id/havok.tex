...
\label{sec:havok}
    ...
    The Koopman operator is a method of representing finite-dimensional nonlinear dynamics
    ...
    This implementation well be refered to as \gls{HAVOKc} 
    \gls{HAVOKc} results in a discrete, linear model that approximates the behaviour of a controlled dynamical system.
    ...
    \begin{figure}[htb]
        ...
        \centering
        \caption{Illustration of the extraction of $\Tilde{\bm{A}}$ and $\Tilde{\bm{B}}$ from Equation~\ref{eq:v_ss_a}}
    ...
    \gls{HAVOKc} does not produce these exact values in $\Tilde{\bm{A}}$ and $\Tilde{\bm{B}}$. 
    ...
    \paragraph
    Finally, the improved $\Tilde{\bm{A}}$ and $\Tilde{\bm{B}}$ are inserted into Equation~\ref{eq:havok_state_space} to render the \gls{HAVOKc} model.
