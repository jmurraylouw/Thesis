\section{Plant considered for system identification} \label{sec:plant_considered}
    ...
    % The model identified by \gls{DMDc} or \gls{HAVOK} will be used to design a longitudinal velocity controller.
    ...
    % As discussed in Section~\ref{sec:linear_model}, the payload minimally effects the mulitrotor attitude because it is attached near the \gls{CoM} of the vehicle.
    ...
    Because of the symmetry of the mulitrotor, this controller can then duplicated for East velocity control.
    ...
    \begin{equation}
        ...
        \end{bmatrix} .
    \end{equation}
    For swing damping control, the controllers require state feedback from both the mulitrotor and payload,
    ...
    Also note that the inner loop controllers handle the attitude dynamics of the mulitrotor.
    During pure longitudinal velocity setpoints the mulitrotor experiences negligible altitude changes due the swinging payload.
    ...
    A schematic of this \gls{2D} plant is shown in Figure \ref{fig:floating_pend}.
    In the following sections, simulations of the full mulitrotor and payload system will be performed
    ...
    % However, $\dot{\theta}$ is not measured directly on the considered practical mulitrotor setup.
