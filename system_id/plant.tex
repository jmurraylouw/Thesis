\section{Plant considered for system identification} \label{sec:plant_considered}
    ...
    % Therefore the attitude state variables of the quadrotor are excluded from the system identification model.
    ...
    \paragraph
    A specific subsystem of the quadrotor-payload system will be considered for system identification.
    Because the proposed controllers in Chapter~\ref{chap:control} will be applied for North velocity control,
    the longitudinal dynamics will be considered in this section, therefore
    ...
    For swing damping control, the controllers will require feedback from both the multirotor and payload variables,
    therefore the state vector of the considered plant is,
    \begin{equation}
        \bm{x} = \begin{bmatrix}
            V_N & \theta & \dot{\theta}
        \end{bmatrix}^T ,
    ...
    and $V_N$ is the North velocity of the quadrotor in the inertial frame. 
    ...
    \paragraph
    In subsequent sections, simulations of the full, non-linear multirotor and payload system will be performed
