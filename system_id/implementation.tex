\section{Implementation and results}
    ...
    \subsection{Length of training data}
        ...
        However, a phenomenon named `double-descent' occurs when the dimension of a regression model 
        is near to the number of training samples \cite{Nakkiran2019}.
        This is called an over-paramregression

        \paragraph{}
        However, it should be noted that the plot for $l =$~\SI{0.5}{\metre} in Figure~\ref{fig:different_lengths}
        ...
        Hence the algorithm needs to be exposed to more step responses to capture the steady-state behaviour of the plant,
        since there is not enough information exposed in the first step responses of the training data.

        \paragraph{}
        Because the data-driven methods are linear regression algorithms, 
        the peak accuracy is reached with a relatively short lengths of data.
        It
