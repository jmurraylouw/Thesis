\section{Implementation and results}
    ...
    \paragraph
    Numerous simulations were performed with different system configurations to investigate the performance of these system identification techniques.
    ...
    \subsection{Methodology} \label{sec:sys_id_methodology}
        ...
        to a safe value where the mulitrotor remains in stable flight
        ...
        The error metric calculated from the testing data is used to evaluate and rank the performances of each models.       
        ...
        % However, for the 
        % In practice it is more common to command position step setpoints since a mulitrotor delivers
    ...
    \subsection{Error metric}
        ...
        However for the mulitrotor and payload case it is assumed that variance error 
        ...
        % For example, the mulitrotor velocity reaches values of \SI{3}{\metre/\second} but the payload swing angle has a maximum of only \SI[]{0.526}{\radian}.
    ...
    \subsection{Hyperparameters} \label{sec:hyperparameters}
        ...
        \gls{DMDc} and \gls{HAVOK} models are dependent on two hyperparameters: 
        ...
        \paragraph
        Figure~\ref{fig:NMAE_vs_q} shows the prediction error of \gls{DMDc} and \gls{HAVOK} models for different values of $q$.
        ...
        There is only a slight difference between the results of \gls{DMDc} and \gls{HAVOK}.
        ...
        \paragraph
        Figure~\ref{fig:singular_values} plots the singular values of the \gls{SVD} from a \gls{HAVOK} model in a log scale.
    ...
    \subsection{Sample time}
        ...
        Due to the computational complexity of an \gls{MPC}, the optimiser will struggle to run at \SI{50}{\hertz} on a companion computer on a mulitrotor.
        However, the controller needs to run as fast as possible 
        to have significant time-scale separation from the mulitrotor dynamics.
        ...
        \paragraph
        Figure~\ref{fig:MAE_vs_Ts_vs_L} shows the prediction error of different \gls{DMD} models 
        ...
        However, this sample time is too slow for controlling the practical mulitrotor.
        $T_s =$~\SI{0.03}{\second} is selected as the sample time for system identification 
        because it provides a good balance between being fast enough for the mulitrotor dynamics and slow enough for a practical \gls{MPC} implementation.
    ...
    \subsection{Choice of payload variable in the state vector}
        ...
        \paragraph
        Figure~\ref{fig:different_state_vectors} shows the prediction error of \gls{HAVOK} models using $\dot{\theta}$ or $\theta$ 
        ...
        since there is no direct measurement of $\dot{\theta}$ on the practical mulitrotor.
    ...
    \subsection{Noise} \label{sec:noise}
        ...
        On the practical mulitrotor the \gls{IMU}, barometer, magnetometer and \gls{GPS} sensors experience measurement noise.
        ...
        \paragraph
        The potentiometer and \gls{ADC} which measure the payload angle on the mulitrotor also has experience measurement noise.
        ...
        The noisy signals from both the mulitrotor \gls{EKF} and payload swing angle are smoothed 
    ...
    \subsection{System parameters} \label{sec:system_params}
        ...
        For the practical mulitrotor considered, the payload mass is limited to,
        $
        ...
        $
        Figure~\ref{fig:system_param_subfigs} shows the prediction error of \gls{HAVOK} models 
        build from simulations with various values of $m_p$ and $l$.
        The plots are not shown for \gls{DMDc} models because they are so similar to the \gls{HAVOK} results.
        ...
        % On the other limit, $m_p = \SI{0.4}{\kilo\gram}$ is determined to be the maximum payload mass the mulitrotor can carry safely 
        ...
        % since the payload may collide with the mulitrotor during an uncontrolled swing.
        ...
        % since a cable much longer than this may not be practically useful for a drone delivery flight with the considered mulitrotor.
    ...
    \subsection{Length of training data} \label{sec:sys_id_length_of_data}
        ...
        Less training data means less flight time will be wasted on training a model before the mulitrotor can fly with a updated controller.
        Less training data also corresponds to lower memory usage on the mulitrotor hardware and lower time-complexity for the algorithm.
    ...
    \subsection{Dynamic payload} \label{sec:dynamic_payload}
        ...
        However, the \gls{DMDc} and \gls{HAVOK} models are still able to approximate the dynamics quite accurately 
        ...
        % Also note that \gls{HAVOK} seems to mostly produces slightly lower errors for low values of $q$,
        % but \gls{DMDc} and \gls{HAVOK} produce very similar
        ...
        % is if a payload is attached rigidly to the mulitrotor while also carrying a suspended payload.
        % The payload mass estimation is based on the assumption that the mulitrotor mass is known.
        % However if a mass is rigidly attached to the vehicle, the effective mulitrotor mass is changed and the \gls{RLS} payload mass estimation is no longer accurate.
